...
if (smtpHost && smtpUser && smtpPassword) {
  transporter = nodemailer.createTransport({
    host: smtpHost,
    port: smtpPort,
    secure: smtpPort === 465, // true pour port 465, false pour autres (287, etc.)
    auth: {
      user: smtpUser,
      pass: smtpPassword,
    },
  ...
  console.warn('⚠️  Variables SMTP manquantes - Les notifications email seront désactivées');
}
...
export async function sendEmailMultiple(
  ...
  try {
    ...
    console.log(`✅ Email envoyé à ${recipients.length} destinataires: ${result.messageId}`);
  } catch (error) {
